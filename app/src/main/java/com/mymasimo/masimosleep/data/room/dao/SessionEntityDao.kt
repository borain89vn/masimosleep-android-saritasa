...
interface SessionEntityDao {
    ...
    @Query("SELECT ${Contract.COLUMN_ID} FROM ${Contract.TABLE_NAME} " +
            "WHERE ${Contract.COLUMN_END_AT} is null " +
            "ORDER BY ${Contract.COLUMN_START_AT} DESC " +
            "LIMIT 1")
    ...
    @Query("SELECT * FROM ${Contract.TABLE_NAME} ORDER BY ${Contract.COLUMN_START_AT} DESC LIMIT 1")
    fun findLatestSession(): Single<SessionEntity>
    ...
    @Query("DELETE FROM ${Contract.TABLE_NAME} " +
            "WHERE ${Contract.COLUMN_PROGRAM_ID} = :programId " +
            "AND ${Contract.COLUMN_END_AT} is null")
