...
interface SessionNoteEntityDao {
    ...
    @Query("SELECT * FROM ${SessionNoteContract.TABLE_NAME} " +
    ...
    fun findAllBySessionId(sessionId: Long): Single<List<SessionNoteEntity>>
    ...
    @Query("SELECT * FROM ${SessionNoteContract.TABLE_NAME} " +
            ...
            "WHERE ${SessionContract.TABLE_NAME}.${SessionContract.COLUMN_ID} = :sessionId " +
            "ORDER BY ${SessionNoteContract.COLUMN_CREATED_AT} ASC")
    fun findAllBySessionIdUpdates(sessionId: Long): Observable<List<SessionNoteEntity>>
}